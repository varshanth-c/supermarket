...
import { AlertTriangle, TrendingUp, TrendingDown, DollarSign, Package, Users, Calendar, Filter, Download, FileText, BarChart3 } from 'lucide-react';
...
import { FinancialMetricsCard } from '@/components/cards/FinancialMetricsCard';
...
const fetchAnalytics = async (timeFilter: string) => {
  const response = await fetch(`http://localhost:8000/api/analytics?period=${timeFilter}`);
  if (!response.ok) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card className="border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-slate-900">
                ${financial_metrics.total_sales?.toLocaleString() || '0'}
              </div>
          ...
          <Card className="border-l-4 border-l-orange-500 hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-slate-900">
                ${financial_metrics.total_expenses?.toLocaleString() || '0'}
              </div>
          ...
          <Card className={`border-l-4 ${financial_metrics.net_profit >= 0 ? 'border-l-green-500' : 'border-l-red-500'} hover:shadow-md transition-shadow`}>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${profitColor}`}>
                ${financial_metrics.net_profit?.toLocaleString() || '0'}
              </div>
              <p className="text-xs text-slate-500 mt-1">
                {financial_metrics.net_profit >= 0 ? 'Your business is profitable' : 'Focus on reducing costs'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-purple-500 hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${profitColor}`}>
                {financial_metrics.profit_margin?.toFixed(1) || '0'}%
              </div>
              <p className="text-xs text-slate-500 mt-1">
                {financial_metrics.profit_margin > 20 ? 'Excellent margins' : 
                 financial_metrics.profit_margin > 10 ? 'Good profitability' : 'Consider cost optimization'}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription className="text-sm text-slate-600">
                  Daily sales trend shows your business momentum. Look for patterns to optimize inventory and staffing.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <SalesOverviewChart data={analytics.daily_sales} />
              </CardContent>
            </Card>
          </div>
          ...
          {/* Inventory Alerts */}
          <div>
            <InventoryAlertsCard data={analytics.low_stock} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-slate-900">Best Performers</CardTitle>
              <CardDescription className="text-sm text-slate-600">
                Your top-selling items. Keep these well-stocked and consider promoting similar products.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TopSellingChart data={analytics.top_selling} />
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-slate-900">Profit Champions</CardTitle>
              <CardDescription className="text-sm text-slate-600">
                Items generating the highest profit margins. Focus marketing efforts on these products.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ProfitabilityChart data={analytics.profitable_items} />
            </CardContent>
          </Card>
        </div>

        {/* Expense and Customer Analysis */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          {/* Expense Breakdown - Takes 2 columns */}
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-slate-900">Where Your Money Goes</CardTitle>
                <CardDescription className="text-sm text-slate-600">
                  Expense breakdown helps identify cost optimization opportunities. Large segments may need attention.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ExpenseBreakdownChart data={analytics.expense_breakdown} />
              </CardContent>
            </Card>
          </div>
          ...
          <div>
            <Card>
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-slate-900">Customer Mix</CardTitle>
                <CardDescription className="text-sm text-slate-600">
                  Balance of returning vs new customers shows business health and growth potential.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <CustomerTrendsChart data={analytics.customer_trends} />
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-slate-900">7-Day Sales Forecast</CardTitle>
              <CardDescription className="text-sm text-slate-600">
                AI-powered prediction of your next week's sales. Plan inventory and staffing accordingly.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div>
            <AssociationRulesCard data={analytics.associations} />
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1">
            ...
            Health Score calculated from profitability, revenue scale, and cost control
          </p>
        </div>
      </div>
    </div>
